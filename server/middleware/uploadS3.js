...
exports.fileUploadMiddlewareS3 = async function (req, res, next) {
    if (req.files) {
        ...
        if (!isValid) {
            ...
            return next({
                message: "One of the files is not a valid video/image file",
            });
        }
                ...
                if (isVideo(req.files[i].path)) {
                    ...
                    await process.then(
                        async function (video) {
                            ...
                            await video
                                .setVideoSize("50%", true, false, "#000000")
                                .save(
                                    `./newFiles/${newName}.mp4`,
                    ...
                    await sharp("./" + req.files[i].path)
                        ...
                        .then(async () => {
                            await uploadFile(
                                `${newName}.png`,
                                `./newFiles/${newName}.png`
                            );

                            console.log("in");
                            await links.push(
                                process.env.CLOUDFRONT_URL + `${newName}.png`
                            );
                            fs.unlinkSync(
                                `./newFiles/${req.files[i].filename}.png`
                            );
        ...
        try {
            for (let i = 0; i < req.files.length; i++) {
                if (isVideo(req.files[i].path)) {
                    ...
                    console.log("in");
                    await links.push(
                        process.env.CLOUDFRONT_URL + req.files[i].filename
                    );
                    fs.unlinkSync("./" + req.files[i].path);
                    if (links.length === req.files.length) {
                        let date = new Date();
